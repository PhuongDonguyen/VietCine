...
export default function SeatSelection() {
  ...
  useEffect(() => {
    const fetchShowtimeDetails = async () => {
      ...
      try {
        ...
        const bookingDate = new Date(showtimeData.startTime)
          .toISOString()
          .split("T")[0];
        ...
        if (!seatTypesResponse.data.success) {
          throw new Error(
            "Could not load seat types: " + seatTypesResponse.data.message
          );
        }
  ...
  const getSeatPrice = (seatTypeId: number): number => {
    const seatPrice = seatPrices.find(
      (price) => price.seatTypeId === seatTypeId
    );
    if (seatPrice) {
  ...
  const handleSeatToggle = (
    ...
    if (seatIndex > -1) {
      setSelectedSeats((prevSeats) =>
        prevSeats.filter((seat) => seat.SeatId !== seatId)
      );
    } else {
  ...
  const initiateVNPayPayment = async (foods: FoodItem[]) => {
    try {
      ...
      const seatNumbersList = selectedSeats
      ...
      const paymentResponse = await axios.post(
        "http://localhost:8081/api/vnpay-payment/create",
        paymentData,
        {
          headers: {
      ...
      if (paymentUrl) {
        ...
        if (!bookingResponse.data.success) {
          throw new Error(
            bookingResponse.data.message || "Không thể tạo booking"
          );
        }
      ...
      alert(
        err.message ||
          "Đã có lỗi xảy ra khi khởi tạo thanh toán VNPAY. Vui lòng thử lại!"
      );
    }
  };
