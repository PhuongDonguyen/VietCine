            ...
            try {
                ...
                if (response.data.success) {
                    ...
                    const applicableVouchers = response.data.data.filter(
                        (voucher: Voucher) => voucher.minBillPrice <= subtotal && !voucher.isUsed
                    );
            ...
            {selectedSeats.length > 0 && (
                <div className="mb-4 border-b border-gray-800 pb-4">
                    ...
                    <div className="relative">
                        ...
                        >
                            <span className={selectedVoucher ? "text-white" : "text-gray-400"}>
                                {loading ? "Đang tải..." :
                                    selectedVoucher ? selectedVoucher.description :
                                        vouchers.length === 0 ? "Không có voucher khả dụng" : "Chọn voucher"}
                            </span>
                            <ChevronDown className={`h-4 w-4 transition-transform ${isVoucherDropdownOpen ? 'rotate-180' : ''}`} />
                        </button>
                                ...
                                {vouchers.map((voucher) => (
                                    ...
                                    >
                                        <div className="flex justify-between items-center">
                                            <div>
                                                <p className="text-white font-medium">{voucher.description}</p>
                                                <p className="text-xs text-gray-400">
                                                    Giảm {formatPrice(voucher.discount)} • Tối thiểu {formatPrice(voucher.minBillPrice)}
                                                </p>
                                            </div>
