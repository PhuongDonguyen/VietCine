...
export function SeatMap({
  ...
  const calculateSeatPositions = () => {
    const maxCol = Math.max(
      ...seats.map((seat) =>
        seat.seatTypeId === 3 ? seat.column + 1 : seat.column
      )
    );
    ...
    <div className="overflow-x-auto bg-gray-900 p-4">
      ...
      <div className="flex flex-col items-center mb-6">
        ...
        {seatRows.map((row) => (
          <div
            key={row.row}
            className="flex items-center mb-1 relative w-full justify-center"
          >
            <div className="w-6 font-bold text-center mr-4 text-white">
              {row.row}
            </div>
            ...
            <div
              ...
              style={{ width: `${maxCol * seatUnitWidth}px` }}
            >
              {row.seats.map((seat) => {
                ...
                return (
                  <div
                    ...
                    className={`absolute ${seatWidth} h-8 rounded ${getSeatClass(
                      seat
                    ...
                    onClick={
                      seat.available ? () => handleSeatClick(seat) : undefined
                    }
                    title={`${displayLabel} - ${getSeatTypeName(
                      seat.seatTypeId
                    )}`}
                    data-testid={`seat-btn-${seat.row}${seat.column}`}
                  >
                    {displayLabel}
                  </div>
                );
