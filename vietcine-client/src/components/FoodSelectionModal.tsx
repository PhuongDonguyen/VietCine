    ...
    const fetchFoods = async () => {
      try {
        ...
        const response = await axios.get(
          `http://localhost:8081/api/food?theaterBrandId=${theaterBrandId}`
        );

        if (response.data.success) {
          // Convert the API food items to FoodItems with quantity
          const foodItems: FoodItem[] = response.data.data.map(
            (food: Food) => ({
              ...food,
              quantity: 0,
            })
          ...
          {loading ? (
            ...
            <div className="text-center py-8">
              <p className="text-gray-400 text-lg">
                Hiện tại rạp chưa bổ sung bắp nước, quý khách vui lòng tiếp tục
                nhé!
              </p>
            </div>
          ) : (
                ...
                >
                  ...
                  <div className="flex items-center space-x-3">
                    <button
                      ...
                      data-testid={`food-minus-btn-${food.id}`}
                    >
                    ...
                    <button
                      ...
                      data-testid={`food-plus-btn-${food.id}`}
                    >
