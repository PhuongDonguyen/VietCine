...
interface Movie {
    ...
    slug: string;
    isAvailable: boolean;
    movieCasts?: MovieCast[]; // Add movieCasts property based on the API response
}
...
export default function MovieDetail() {
    ...
    // Removed separate fetch for cast since we now get it from the movie data
    ...
    return (
        <div className="min-h-screen bg-black text-white flex flex-col">
            <NavBar transparent={true} fixedTop={true} />
                        ...
                        <div>
                            ...
                            {loadingCast ? (
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                                    {movie.movieCasts.map((castItem, index) => (
                                        <div key={index} className="flex flex-col items-center">
                                            <div className="w-20 h-20 mb-2 overflow-hidden rounded-full border-2 border-gray-800">
                                                <img
                                                    src={castItem.cast.avatar || "/default-avatar.jpg"}
                                                    alt={castItem.cast.name}
                                                    className="w-full h-full object-cover"
                                                />
                                            </div>
                                            <h4 className="font-medium text-center text-sm">{castItem.cast.name}</h4>
                                            <p className="text-xs text-gray-400 text-center">vai {castItem.characterName}</p>
                                        </div>
                                    ))}
                                </div>
                            ) : (
